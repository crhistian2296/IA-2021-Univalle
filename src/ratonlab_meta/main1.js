...
myProblem.solve(
    ...
    ],
    {
        onFinish: (result) => {
            ...
            if (status == -1) console.log("Agent cannot solve this problem :(");
            else console.log("Agent could solve this problem :)");
        },
        onTurn: (result) => {
            console.log(
                "Turn: " +
                    JSON.stringify(result.actions[result.actions.length - 1])
            );
        },
    }
);
