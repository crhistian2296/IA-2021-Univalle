...
/* function avoidCycles(nodo, nodos) {
    let len = nodo.boxes.length;
    let contador = 0;
    for (let i = 0; i < nodos.length; i++) {
        if (nodo.value.x == nodos[i].value.x && nodo.value.y == nodos[i].value.y) {
            for (let j = 0; j < len; j++) {
                if (nodo.boxes[j].x == nodos[i].boxes[j].x && nodo.boxes[j].y == nodos[i].boxes[j].y) {
                    contador++;
                }
            }
            if (contador == len) {
